...
DEFAULT_WORKSPACE_ROOT = "/workspace/.mcpworld/vscode/C-Plus-Plus"
...
def message_handler(message: Dict[str, Any], logger, task_parameter: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:    
    ...
    workspace_root = message.get('root') or DEFAULT_WORKSPACE_ROOT
    if event_type == "get_root_when_start":
        dir_path = os.path.join(workspace_root, expected_relative_path)
        try:
        ...
        try:
            ...
            return [{"status": "error", "type": "evaluate_on_completion", "message": f"Directory not found: {dir_path}"}]
