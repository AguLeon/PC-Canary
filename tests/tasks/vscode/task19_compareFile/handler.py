    ...
    if event_type == "evaluate_on_completion":
        ...
        if expected_file1 in files and expected_file2 in files:
            return [
                ...
                {"status": "success", "reason": "Compare view is open for the two expected files"}
            ]
        else:
            return [{"status": "error", "type": "evaluate_on_completion", "message": "Compare view does not match expected files"}]
    return None
