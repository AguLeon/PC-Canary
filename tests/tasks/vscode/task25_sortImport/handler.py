...
def message_handler(message: Dict[str, Any], logger, task_parameter: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:    
    ...
    expected_file_path = task_parameter.get("expected_file_path", "/workspace/.mcpworld/vscode/C-Plus-Plus/python_test/sort_import.py")
    ...
    if event_type == 'evaluate_on_completion' and fileName == expected_file_path and hasErrors == False:
        if ''.join(content.split()) == expected_file_content:
            return [
                ...
                {"status": "success", "reason": "Import statements sorted and file matches expectation"}
            ]
        else:
            return [{"status": "error", "type": "evaluate_on_completion", "message": "File content does not match expected import ordering"}]
    return None
