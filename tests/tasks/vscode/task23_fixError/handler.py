...
def message_handler(message: Dict[str, Any], logger, task_parameter: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:    
    ...
    expected_file_path = task_parameter.get("expected_file_path", "/workspace/.mcpworld/vscode/C-Plus-Plus/agent_test/fix_error.cpp")
    logger.info(message)
    if event_type == 'evaluate_on_completion' and fileName == expected_file_path and hasErrors == False:
        if ''.join(content.split()) == expected_file_content:
            return [
                ...
                {"status": "success", "reason": "File matches the expected fixed content and has no errors"}
            ]
        else:
            return [{"status": "error", "type": "evaluate_on_completion", "message": "File contents or diagnostics still do not match expectations"}]
    return None
