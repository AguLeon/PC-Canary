...
def create_model(model_type, api_key):
    ...
    if model_type.lower() == "openai":
        return OpenAIModel(
            api_key=api_key, model_name="gpt-4o-mini", temperature=0.2, max_tokens=2048
        )
...
def test_agent_telegram_search(model_type="openai", api_key=None, max_steps=10):
    ...
    # Create Agent
    agent = BaseAgent(
        model, observation_type="screenshot", action_space="pyautogui-muti-action"
    )
    ...
    while (
        ...
        and not controller.task_completed
        and not controller.task_failed
    ):
        ...
        if action == "finish":
            ...
            controller.failure_reason = (
                args.get("reasoning", "Agent reported FAIL")
                if args
                else "Agent reported FAIL"
            )
            break  # Exit loop

        elif action is None:
            print(
                f"Agent decision or parsing error: {args.get('error', 'Unknown error')}"
            )
            controller.task_failed = True
            controller.failure_reason = (
                args.get("error", "Agent act returned None")
                if args
                else "Agent act returned None"
            )
            break  # Exit loop
    ...
    print(
        f"Execution status: {'Success' if controller.task_completed else 'Failure' if controller.task_failed else 'Not completed'}"
    )
    ...
    print(f"Report file: {os.path.join(logger.session_dir, 'session_report.md')}")
...
def main():
    ...
    parser.add_argument(
        "--model",
        choices=["openai", "gemini", "qwen", "claude"],
        default="claude",
    ...
    try:
        ...
        success = test_agent_telegram_search(
            model_type=args.model, api_key=args.api_key, max_steps=args.max_steps
        )
